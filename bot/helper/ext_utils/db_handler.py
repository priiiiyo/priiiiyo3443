...
class DbManger:
    ...
    def addleech_log_alt(self, chat_id: int):
        if self.err:
            ...
            sql = f'INSERT INTO users (uid, leechlog_alt) VALUES ({chat_id}, TRUE)'
        else:
            sql = f'UPDATE users SET leechlog_alt = TRUE WHERE uid = {chat_id}'
        self.cur.execute(sql)
        self.conn.commit()
        self.disconnect()
        return '𝗦𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆 𝗮𝗱𝗱𝗲𝗱 𝘁𝗼 𝗹𝗲𝗲𝗰𝗵 𝗹𝗼𝗴𝘀 😁'
    ...
    def rmleech_log_alt(self, chat_id: int):
        if self.err:
            ...
            sql = f'UPDATE users SET leechlog_alt = FALSE WHERE uid = {chat_id}'
            self.cur.execute(sql)
            self.conn.commit()
            self.disconnect()
            return '𝗥𝗲𝗺𝗼𝘃𝗲𝗱 𝗳𝗿𝗼𝗺 𝗹𝗲𝗲𝗰𝗵 𝗹𝗼𝗴𝘀 𝘀𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆 😁'
